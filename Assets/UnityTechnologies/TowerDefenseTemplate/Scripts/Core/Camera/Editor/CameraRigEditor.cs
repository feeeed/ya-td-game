...
namespace Core.Camera.Editor
{
	...
	public class CameraRigEditor : UnityEditor.Editor
	{
		...
		void OnSceneGUI()
		{
			...
			var fmh_42_60_638610738569670908 = Quaternion.LookRotation(Vector3.up); Vector3 bottom = Handles.FreeMoveHandle(bottomPosition, size, snap,
			                                        Handles.RectangleHandleCap);
			var fmh_44_54_638610738569688595 = Quaternion.LookRotation(Vector3.up); Vector3 top = Handles.FreeMoveHandle(topPosition, size, snap,
			                                     Handles.RectangleHandleCap);
			var fmh_46_56_638610738569694639 = Quaternion.LookRotation(Vector3.up); Vector3 left = Handles.FreeMoveHandle(leftPosition, size, snap,
			                                      Handles.RectangleHandleCap);
			var fmh_48_58_638610738569699553 = Quaternion.LookRotation(Vector3.up); Vector3 right = Handles.FreeMoveHandle(rightPosition, size, snap,
			                                       Handles.RectangleHandleCap);
